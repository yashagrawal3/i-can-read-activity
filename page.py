...
class Page():
    ...
    def __init__(self, canvas, lessons_path, images_path, sounds_path, level,
        ...
        # csurface = cairo.ImageSurface(cairo.FORMAT_ARGB32,int(self._width),int(self._height))
        # self._my_canvas = Sprite(self._sprites, 0, 0, csurface)
        # self._my_canvas.set_layer(0)
        # self.cr = self._my_canvas.images[0].new_gc()
        # self.cr.set_foreground(
        #     self.cr.get_colormap().alloc_color('#FFFFFF'))

        self._my_canvas = Sprite(
            self._sprites, 0, 0, svg_str_to_pixbuf(genblank(
                    self._width, (self._height * 2.75), (self._colors[0],
                                                self._colors[0]))))
        ...
        for c in ALPHABET:
            self._letters.append(Sprite(self._sprites, 0, 0,
                svg_str_to_pixbuf(generate_card(string=c.encode('ascii','ignore'),
    ...
    def page_list(self):
        ...
        self._my_canvas.images[0].draw_rectangle(self.cr, True, *rect)
            ...
            if type(self._color_data[self.page][0]) == type([]):
                ...
                self._cards.append(Sprite(self._sprites,
                    int(self._width - 320 * self._scale / 2.5),
                                ...
                                string=('%s' % (self._card_data[self.page][0].lower())),
    ...
    def _draw_pixbuf(self, pixbuf, x, y, canvas, cr):
        ...
        Gdk.cairo_set_source_pixbug(cr, pixbuf, 0 , 0)
        self.invalt(x, y, w, h)
    ...
    def invalt(self, x, y, w, h):
        ...
        # self._canvas.get_property('window').invalidate_rect(
